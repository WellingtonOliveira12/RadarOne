...
/**
 * Webmotors Scraper - Implementação Real
 *
 * Extrai anúncios de veículos do portal Webmotors
...
/**
 * Executa scraping na Webmotors com rate limiting e retry
 */
export async function scrapeWebmotors(
  monitor: MonitorWithFilters
): Promise<ScrapedAd[]> {
  // Aplica rate limiting
  await rateLimiter.acquire('WEBMOTORS');

  // Executa scraping com retry
  return retry(() => scrapeWebmotorsInternal(monitor), retryPresets.scraping);
}

/**
 * Implementação interna do scraping (usada pelo retry)
 */
async function scrapeWebmotorsInternal(
  monitor: MonitorWithFilters
): Promise<ScrapedAd[]> {
  console.log(`🔍 Starting Webmotors scraper for: ${monitor.name}`);
  ...
  try {
    ...
    console.log(`✅ Extracted ${ads.length} ads from Webmotors`);

    await browser.close();
    return ads;
  } catch (error: any) {
    console.error(`❌ Error in Webmotors scraper: ${error.message}`);
  ...
  const rawAds = await page.$$eval(
    ...
    (elements) => {
      return elements
        .map((el) => {
          try {
            // Extract title (marca + modelo + ano)
            const titleEl = el.querySelector('[data-testid="card-title"]');
            const title = titleEl?.textContent?.trim() || '';

            // Extract price
            const priceEl = el.querySelector('[data-testid="best-price"]');
            const priceText = priceEl?.textContent?.trim() || '';
            // Remove "R$" e converte (ex: "R$ 45.900" -> 45900)
            ...
            const url = href.startsWith('http')
              ? href
              : `https://www.webmotors.com.br${href}`;
            ...
            // Extract location
            const locationEl = el.querySelector('[data-testid="card-location"]');
